...
namespace MinMax_Slider
{
	...
	public class MinMaxSlider : Selectable, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		[Header("Duration Threshold")]
		[SerializeField] private float durationMin = 5;
		[SerializeField] private float durationMax = 15;
		...
		private void UpdateText()
		{
			...
			UIDisplayManager.s.setClipDuration(duration,durationMin, durationMax);				//kinda ugly but updates all the relevant values at once
			clipDuration.text = "Selection Duration: " + (duration).ToString() + " seconds";
			//NOTE: set duration threshold here for valid or invalid
